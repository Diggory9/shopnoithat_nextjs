            ...
            {categories
                ...
                .map((category) => {
                    const hasChildren = categories.some(
                        (child) => child.categoryParent === category.id
                    );
                    return (
                        <li key={category.id} className="w-full relative">
                            <button
                                className="w-full rounded-t  dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-700 py-2block whitespace-no-wrap text-start"
                                onClick={() =>
                                    handleOnSelectCategory(category.id)
                                }
                            >
